...
async function fetchTrafficFromSheet() {
  ...
  try {
    ...
    statusEl.textContent = 'Error: ' + e.message + ' — Sheet কি publicly shared আছে?';
...
function parseMnoCsv(input) {
  readFile(input, (text, name) => {
    try {
      ...
      // Count by gateway × ansResponseCode
      const opCodes = {}; // { GP: { '1003': 5, ... }, ... }
      let total = 0;
      rows.forEach(row => {
        const op = gwToOp(row.applicableSmsGateway);
        const code = row.ansResponseCode;
        if (!code || code === 'null' || code === '-') return;
        const codeNum = parseInt(code);
        if (isNaN(codeNum) || codeNum < 1000 || codeNum > 1099) return;
        if (!op) return;
        if (!opCodes[op]) opCodes[op] = {};
        opCodes[op][code] = (opCodes[op][code] || 0) + 1;
        total++;
      });

      updateCsvPreview('MNO 1xx', opCodes, total);
      setStatus('csv-mno-status', `Parsed ${total} 1xx records`, 'ok');
      showToast('MNO 1xx CSV loaded');
    } catch(e) {
      setStatus('csv-mno-status', 'Parse error: ' + e.message, 'error');
    }
  });
}
  ...
  readFile(input, (text, name) => {
    try {
      ...
      rows.forEach(row => {
        ...
        const codeNum = parseInt(code);
        if (isNaN(codeNum) || codeNum < 1000 || codeNum > 1099) return;
        if (!gwCodes[gw]) gwCodes[gw] = {};
        gwCodes[gw][code] = (gwCodes[gw][code] || 0) + 1;
        total++;
      });

      updateCsvPreview('IPTSP 1xx', gwCodes, total);
      setStatus('csv-iptsp-status', `Parsed ${total} 1xx records across ${Object.keys(gwCodes).length} gateways`, 'ok');
      showToast('IPTSP 1xx CSV loaded');
    } catch(e) {
      setStatus('csv-iptsp-status', 'Parse error: ' + e.message, 'error');
    }
  });
}
